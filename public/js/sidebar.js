{
            ...
            if (subHeader.classList.contains("clickable")) {
              if (subHeader.nextElementSibling) {
                let bodyHeight =
                  ...
                  subHeader.nextElementSibling.getBoundingClientRect().top -
                  20;
    ...
    if (dropDownItemParent !== undefined) {
      function getParent() {
        if (
          ...
          dropDownItemParent =
            dropDownItemParent.parentElement.parentElement.parentElement;
  ...
  function removeElement(data, prevArray) {
    let elements =
      data.parentElement.parentElement.querySelectorAll(".drop-down-header");
    elements.forEach((el) => {

{
    let fetchData = async function fetchData(
        route,
        form,
        relations = [],
        options = {}
    ...
    function insertValueOnField(index, form, data, fetchOptions) {
        ...
        if (field.getAttribute("type") == "file") {
            ...
            if (parent) {
                ...
                let optionItems = parent.querySelectorAll(
                    ".select-items > div"
                );
                ...
                try {
                    if (fetchOptions["choice_instance"]) {
                        let id = field.id;
                        let selecteInstant =
                            fetchOptions["choice_instance"][id];
                ...
                if (specialInput) {
                    ...
                    if (!isAtLestOneCheck) {
                        if (specialInput[0] && specialInput[0].dataset.name) {
                            let name = specialInput[0].dataset.name.replace(
                                /\[\]/g,
                                ""
                            );
